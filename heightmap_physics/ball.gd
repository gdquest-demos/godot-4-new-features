...
func _request_impact():
	var vel = clamp(linear_velocity.length(), 0.0, max_impact_vel)
	if vel < min_impact_vel: return
	var value = remap(vel, 0.0, max_impact_vel, 0.0 , 1.0)
	
	var t = create_tween()
	t.tween_property(mesh_instance.material_override, "emission_energy_multiplier",
	remap(value, 0.0, 1.0, 0.5, 1.5), 
	remap(value, 0.0, 1.0, 0.1, 0.25))
	t.tween_property(mesh_instance.material_override, "emission_energy_multiplier", 0.0, 0.1)
